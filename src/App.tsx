...
function App() {
  ...
  const [value, setValue] = useState(''); // 富文本的内容
  ...
  /** 初始化页面 - 设置富文本 */
  useEffect(() => {
    setValue(handleLargeText(largeText));
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    timer = setTimeout(() => {
      ...
      iframeEditBtn =
        editorBody?.getElementsByClassName('editor-quote-btn')?.[0];
      ...
      iframeEditBtn?.addEventListener?.('click', function (event) {
        ...
        setValue(newText + originalValue.current); // 设置新的值
  ...
  const handleSave = (e?: any) => {
    ...
    const realValue = e?.getContent?.() || value;
    const mergetText = originalValue.current
      ? realValue.split(largeTextSeparator)?.[0] + originalValue.current
      : realValue;
